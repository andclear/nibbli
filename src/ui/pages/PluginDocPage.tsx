...
/**
 * 面向 AI 的插件开发规范原始文本
 * 用户点击"复制给 AI"时复制的就是这段原始 Markdown
 */
const RAW_DOC = `# 小兔几插件开发规范
...
/**
 * 插件开发文档页面
 * 独立的全屏页面，通过新标签页打开，不使用应用的 Layout 包裹
 */
export function PluginDocPage() {
    const [copied, setCopied] = useState(false);

    /** 复制原始 Markdown 内容到剪贴板 */
    const handleCopy = async () => {
        try {
            await navigator.clipboard.writeText(RAW_DOC.trim());
            setCopied(true);
            toast.success('已复制文档内容，可直接粘贴给 AI');
    ...
    return (
        <div className="min-h-screen bg-background text-foreground">
            ...
            {/* 文档正文渲染区 */}
            <article className="max-w-4xl mx-auto px-6 py-10 prose prose-sm dark:prose-invert prose-headings:scroll-mt-20 prose-table:text-sm prose-th:text-left prose-code:before:content-none prose-code:after:content-none">
